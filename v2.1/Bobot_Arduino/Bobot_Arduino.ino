...
String runInference(float* input) {
    ...
    // Debug
    // Serial.printf("Inference Start. In: %p, W1: %d, B1: %d, Hid: %d, InDim: %d\n", 
    //               input, model.w1.size(), model.b1.size(), model.hidden_dim, model.input_dim);
    
    // L1
    std::vector<float> h1 = dense_ptr(input, model.w1, model.b1, model.hidden_dim, true, model.input_dim);
    
    // Debug
    // Serial.println("Inference L2...");
...
// 160x120 input * 64 hidden + 64 hidden * 2 output + biases
// (19200 * 64) + 64 + (64 * 2) + 2 = 1228800 + 64 + 128 + 2 = 1228994 params
// Compressed (Int8) size: ~1.23MB
// Uncompressed (Float) RAM size: ~4.9MB (Might be tight on 8MB PSRAM with Camera FB? Let's check)
// FB (QVGA, Gray) = 76KB.
// 4.9MB + 1.2MB (buffer) + heap... It fits!
...
void executeAction(JsonObject action) {
    ...
    // Add 'Wait' command? No, rule-based is reactive (runs every frame)
}
...
void runScript(String ai_class) {
    ...
    for (JsonObject rule : rules) {
        const char* condition = rule["if"]; // "Run", "Wait"
        
        // Check match
        if (String(condition) == ai_class) {
            executeAction(rule["do"]);
            return; // Execute first matching rule only? Or all? Let's do first.
        }
        // Handle "Always" or "Else" if needed later
    }
}
...
void handleCommand(String json) {
    ...
    if (strcmp(cmd, "S") == 0) {
        ...
        setMotor(doc["l"], doc["r"]);
         ...
         // Load Script
         // The script comes as a string in "val" or we parse the full array?
         // BLE has limit 517 bytes. A large script might not fit in one packet.
         // But our rules are short.
         // Format: {"cmd":"SCRIPT", "rules":[...]}
         
         scriptDoc.clear();
         // Deep copy rules to scriptDoc
         // We need to parse the *inner* array. 
         // Simpler: Just store the whole received JSON if it fits? 
         // Or better: The user sends JUST the rules array as a separate upload mode?
         // Let's assume small scripts fit in 'doc'.
...
void setup() {
    Serial.begin(115200);
    Serial.println("Bobot v2.0 Starting...");
...
void loop() {
    ...
    if (camera_is_init) {
        ...
        if (fb) {
            ...
            // A. Inference (Native 160x120 -> 160x120)
            if (model.loaded && ai_input_buffer != NULL) {
                 ...
                 if (script_loaded) {
                     ...
                     // Hardcoded Fallback (optional, can be removed)
                     // if (result == "Run") setMotor(60, 60); 
                     // else if (result == "Wait") setMotor(0, 0); 
                 }
            }
